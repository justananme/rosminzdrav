...
export default function Log(props) {
    ...
    const LinearHeader = props => {
        ...
        if (!props.zeroHeader) {
            const testProgress = props.type === TEST_TYPE.TEST ? props.status : 100;
            const finalProgres = props.type === TEST_TYPE.FINAL ? props.status : 0;

            if (props.type === TEST_TYPE.TEST && props.status === 100) {
